...
function PostCmpn (this: any){   
  ...
  function getTimeElapsedSince(dateString: string): string {
    ...
    const getData = async () => {
        ...
        const PostResponse = await getPosts(currentPageNumber, 5,"desc") as unknown as Page_Next_Previous;
...
const fetchMore : Fn = async () =>{
  ...
  async function fetchMoreData () {
        const getData = async () => {
            const PostResponse = await getPosts(currentPageNumber + 1, 5,"desc") as unknown as Page_Next_Previous;
            if(PostResponse != null)
            {
                setData({
    ...
    return haiku.replace(/\n/g, "<br />")
        ...
        return (
            ...
            >
                ...
                <div>
                {data.data.map(post => (
                    <div key={post.Id} className="main rounded bg-white border row" style={{marginBottom:40,paddingLeft:35,border:5}}>
